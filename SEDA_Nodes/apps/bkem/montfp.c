...
*/
  mp = malloc(new_alloc * 4); __GMP_REALLOCATE_FUNC_LIMBS (PTR(m), ALLOC(m), new_alloc);
  m->_mp_d= mp;//PTR(m) = mp;
  m->_mp_alloc = new_alloc;// ALLOC(m) = new_alloc;
...
void * mpz_realloc (mpz_t r, mp_size_t size);
...
static void fp_to_mpz(mpz_ptr z, element_ptr e) {
  ...
  if (!ep->flag) mpz_set_ui(z, 0);
    ...
    mpz_realloc(z, p->limbs);
//	printf("NEED TO IMPLEMENT this !!!!!!!!!!!! fp_to_mpz limbs=%d\n",p->limbs);
...
static int fp_is_sqr(element_ptr a) {
  ...
  if (!ad->flag) return 1;
  fp_to_mpz(z, a);
  printf("NEED TO IMPLEMENT this !!!!!!!!!!!! 2\n");
  return 0;
